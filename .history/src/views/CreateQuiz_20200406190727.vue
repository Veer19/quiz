...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      create(){
        this.quizObj.questions.forEach((q,index)=>{
            q.number = index+1
        })
        console.log(this.quizObj)
        console.log(this.answers)
        firebaseApp.db.collection('tests').add(this.quizObj).then(snapshot=>{
            console.log(snapshot)
            console.log(snapshot.id)
            firebaseApp.db.doc('answers/'+ snapshot.id).set(this.answers).then(snapshot=>{
               this.$notify({
                type: 'success',
                title: 'Quiz Created'
                }) 
